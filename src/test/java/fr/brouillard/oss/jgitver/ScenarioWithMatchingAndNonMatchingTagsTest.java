...
public class ScenarioWithMatchingAndNonMatchingTagsTest {
    ...
    public static void initClass() {
        scenario = Scenarios.s6_matching_and_non_matching_versions_tags();
        if (Misc.isDebugMode()) {
    ...
    @Test
    public void version_on_C() {
        // when tag is checkout
        unchecked(() -> git.checkout().setName("a3.0").call());
        // the version matches the tag
        assertThat(versionCalculator.getVersion(), is("2.0.0-1"));
    }
    
    @Test
    public void version_on_D() {
        // when tag is checkout
        unchecked(() -> git.checkout().setName("dummy").call());
        // the version matches the tag
        assertThat(versionCalculator.getVersion(), is("2.0.0-2"));
    }
    
    @Test
    public void version_on_master() {
        // when tag is checkout
        unchecked(() -> git.checkout().setName("master").call());
        // the version matches the tag
